...
1. =components/u-boot/= 下的源码是一个参考,到最终编译的时候,会被拷贝至 =/build/linux/u-boot/src= 
...
- udevice 指设备对象, 实际上就是一个结构体,在使用设备树的情况下,此对象通过解析设备树而自动生成
- driver 指的是 udevice 的驱动,并且操作函数格式满足 uclass 接口,最终的操作函数会做为uclass的元素
- uclass 是指代表一个设备类, 将同一类的设备操作方法抽象出来.实际上可以理解为包含函数指针的结构体
- uclass_driver 是对 uclass 的操作函数
